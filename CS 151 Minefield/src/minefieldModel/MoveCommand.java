...
public class MoveCommand extends Command {
	...
	public void execute() {
		...
		case "SW":{
			if(p.getX()-1 == -1 || p.getY()+1 == g.getGridSize()+1) {
				Utilities.inform(beyond);
				break;
			}
			x -= 1;
		...
		case "SE":{
			if(p.getX()+1 == g.getGridSize()+1 || p.getY()+1 == g.getGridSize()+1) {
				Utilities.inform(beyond);
				break;
			}
			x += 1;
		...
		case "NE":{
			if(p.getX()+1 == g.getGridSize()+1 || p.getY()-1 == -1) {
				Utilities.inform(beyond);
				break;
			}
			x += 1;
